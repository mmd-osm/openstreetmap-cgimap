...
readonly_pgsql_selection::get_roles_for_user(osm_user_id_t id) {
  std::set<osm_user_role_t> roles;

  m.prepare("roles_for_user",
    "SELECT role FROM user_roles WHERE user_id = $1");

  pqxx::result res = m.exec_prepared("roles_for_user", id);
...
std::optional<osm_user_id_t> readonly_pgsql_selection::get_user_id_for_oauth2_token(const std::string &token_id, bool& expired, bool& revoked, bool& allow_api_write) {
  ...
  pqxx::result res = m.exec_prepared("oauth2_access_token", token_id);
